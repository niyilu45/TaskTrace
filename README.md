...
- Windows 常驻置顶悬浮窗、搜索、托盘及完整界面入口；支持在设置或托盘菜单中启用开机启动，默认关闭；支持树形或“当前任务 / 父任务”单行路径显示，单行模式只保留没有可见子任务和未完成遗留事项的任务行，遗留事项使用粗体。
- 可配置自动保存间隔和数据保存目录。
...
- 多人协作任务在展示模式的任务名下显示协作人数，可展开查看成员头像和用户名；团队更新与冲突统一显示在顶部通知铃铛列表中。
...
下载 Source code ZIP 或使用 Git 克隆源码后，双击 `dist/Install-TaskTrace.cmd`。安装工具不依赖 `.git` 信息，也不要求安装 Git；它会重新读取 Windows 保存的用户与系统 PATH，并从 PATH、`where`、注册表和常见安装位置检查 Node.js 24、pnpm 11.26、Go 1.27、Windows x64 GCC 与 .NET Framework C# 编译器，在日志中写明实际采用的路径。如果缺少依赖，会弹出具体问题、检查范围、安装方法和日志位置；生成成功或失败时也会弹出结果，双击运行不会再因黑框关闭而看不到结果。需要联网补充项目依赖时，安装器会读取 Windows 系统代理，实时显示正在下载的依赖、大小、速度和累计进度；完整清单生成在 `dist/install-dependencies.txt`。
