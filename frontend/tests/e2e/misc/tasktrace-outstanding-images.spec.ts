...
test('outstanding images support paste, append, numbered display and persistence without becoming daily progress', async ({page, request}) => {
	...
	try {
		...
		const raceTask = await (await request.post(`${base}/api/v2/projects/${project.id}/tasks`, {headers, data: {title: '首次遗留事项与每日自动保存并发验收'}})).json()
		await page.setViewportSize({width: 1280, height: 900})
		await page.goto(`${base}/tasks/${raceTask.id}`)
		...
		await daily.getByLabel('启用自动保存').check()
		await daily.getByLabel('今日进展', {exact: true}).fill('图片保存完成后再保存这条每日进展。')
		let releaseUpload!: () => void
		const uploadGate = new Promise<void>(resolve => { releaseUpload = resolve })
		const uploadUrl = `${base}/api/v2/tasks/${raceTask.id}/attachments`
		await page.route(uploadUrl, async route => { await uploadGate; await route.continue() })
		await pastePicture(shared.getByRole('textbox', {name: '新增遗留事项', exact: true}))
		await shared.getByRole('button', {name: '添加遗留事项', exact: true}).click()
		await expect(daily.getByRole('button', {name: '保存进展', exact: true})).toBeDisabled()
		await daily.evaluate(form => form.dispatchEvent(new Event('submit', {bubbles: true, cancelable: true})))
		await page.waitForTimeout(5500)
		const pending = await (await request.get(`${base}/api/v2/tasks/${raceTask.id}/comments`, {headers})).json()
		expect(pending.items).toHaveLength(0)
		await daily.getByLabel('启用自动保存').uncheck()
		releaseUpload()
		await expect(shared.locator('ol.outstanding-list > li')).toHaveCount(1)
		await expect(daily.getByRole('button', {name: '保存进展', exact: true})).toBeEnabled()
		...
		await expect(shared.locator('ol.outstanding-list > li')).toHaveCount(1)
		await expect.poll(() => shared.locator('.readonly-rich-text img').first().evaluate((image: HTMLImageElement) => image.naturalWidth)).toBe(320)
		await expect(daily.getByLabel('今日进展', {exact: true})).toHaveValue('图片保存完成后再保存这条每日进展。')
	} finally {
