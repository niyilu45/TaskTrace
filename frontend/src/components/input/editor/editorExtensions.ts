...
export function createEditorExtensions(deps: EditorExtensionDeps): Extensions {
	...
	const CustomImage = Image.extend({
		...
		renderHTML({HTMLAttributes}) {
			const storedImageUrl = HTMLAttributes['data-src'] ?? HTMLAttributes.src
			const relativeAttachment = typeof storedImageUrl === 'string' && /^\/api\/v[12]\/tasks\/\d+\/attachments\/\d+$/.test(storedImageUrl)
			const imageUrl = relativeAttachment ? window.API_URL + storedImageUrl.replace(/^\/api\/v[12]/, '') : storedImageUrl
			if (imageUrl?.startsWith(window.API_URL)) {
