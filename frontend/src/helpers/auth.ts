import {isLocalWorkspace, LOCAL_REFRESH_KEY} from './tasktraceLocal'
...
async function doRefresh(persist: boolean): Promise<void> {
	...
	const refreshUnderLock = async () => {
		...
		if (isLocalWorkspace()) {
			const refreshBefore = localStorage.getItem(LOCAL_REFRESH_KEY)
			const {oauthToken} = await import('@/client/generated')
			const result = await oauthToken({headers: {Authorization: ''}, body: {
				grant_type: 'refresh_token',
				refresh_token: refreshBefore!,
			}})
			if (loggedOutSinceStart()) return
			const {access_token: accessToken, refresh_token: freshRefreshToken} = result.data
			if (!accessToken || !freshRefreshToken) throw new Error('Invalid local session response')
			localStorage.setItem(LOCAL_REFRESH_KEY, freshRefreshToken)
			saveToken(accessToken, persist)
			return
		}
