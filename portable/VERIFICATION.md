...
## 2026-09-20 悬浮窗单行叶子展示与完成项隐藏延时

- 单行显示只保留最底层可操作内容：任务存在可见子任务或未完成遗留事项时不再单独显示该任务；遗留事项作为单行项目显示，并保留所属任务及上级任务的灰色路径。
- “显示已完成”关闭时，历史完成任务和遗留事项立即隐藏；设置中的隐藏延时只作用于当前悬浮窗会话内刚切换为完成的条目，取消完成或延时到期后同步更新。
- 原生自检覆盖历史完成项、刚完成项、延时到期、单行父任务抑制、遗留事项路径、完成勾选框以及完整/简洁模式切换时的选中项保留。

## 2026-09-20 概览紧凑布局与团队通知归并

- 概览任务列表复用编辑列表的任务行，移除每个层级上下文任务额外占用的一行说明文字，保留项目和最多五级任务的展开/收起。
- 页面顶部通知铃铛直接列出团队成员、任务名、更新时间和待处理冲突；原独立团队铃铛移除，导入任务链接入口保留，点击团队通知仍可进入完整处理窗口。
- 项目展示模式在多人协作任务名下显示协作人数，展开后显示成员头像和用户名；每日进展的作者显示规则保持按实际修改记录决定。
- 相关 Vue 文件通过 ESLint、Stylelint 和 Impeccable 界面机械检查；全库类型检查仍包含既有上游类型错误，未发现本次新增组件的类型错误。

## 2026-09-20 安装器系统代理与依赖下载进度

- 安装器分别针对 npm 注册表和 Go 模块代理读取 Windows 系统代理，并将代理传给 pnpm、Go 及其直接源码下载；日志中的代理地址会移除凭据。
- 每次构建生成 `dist/install-dependencies.txt`，本次锁文件列出 1503 项前端依赖和 196 项 Go 模块；控制台输出改为实时写入 `dist/install.log`。
- pnpm 下载事件显示当前包名、大小、单包速度、累计体积和平均速度，全部命中缓存时明确显示没有联网下载；使用独立空缓存通过系统代理下载 3.6 KB 测试包，包名与实时速度输出正确。
- Go 下载前先区分缓存和缺失模块，缺失模块逐项显示名称、序号、大小及速度；代理瞬时 EOF 会自动重试三次。
- 使用 Windows 系统代理 `127.0.0.1:7897` 实际补充 34 项 Go 缓存，途中一次 EOF 后续构建继续补充剩余 6 项并成功生成完整免安装程序。

## 2026-09-20 源码安装器重新读取工具路径
...
- 项目展示模式增加“最近进展”筛选，支持不限、1/7/30 天和自定义天数；按每日进展的填写日期及本机当天计算自然日范围，普通评论和未来日期不算命中。
- 命中子任务时保留并默认展开完整父任务链，未命中的同级分支不显示；状态和名称筛选可继续组合使用。
- 辅助函数测试覆盖父链、同级隔离、月份/年份/闰日边界、无效日期和未来日期；ESLint、Stylelint、前端构建及本机打包通过。

## 2026-09-19 简洁模式长标题自适应换行

- 简洁模式的树状展示不再把较长的任务和遗留事项直接压成单行省略号；标题会按照悬浮窗当前宽度显示为最多两行，极端长标题仍可横向滚动查看。启用“单行显示”时保持紧凑的横向路径布局。
- 调整窗口宽度、展开或收起任务、数据发生变化后都会重新计算行高；窗口变宽后行高会同步缩回，无变化的后台检查不会触发重绘。
- 原生自检覆盖窄窗口长标题、多级任务、优先级链接、图片链接和横向滚动交互。
...
## 2026-09-18 历史进展多选与双向引用
- 悬浮窗日期月历改为可失焦关闭，点击日期窗口外会取消选择；完整界面保留相同的外部点击关闭行为。两端已有进展的日期继续显示蓝点。
- “引用历史进展”改为日期、历史进展信息和复选框表格，可一次勾选多条并通过“确定/取消”完成或放弃选择。
- 保存后的引用快照可展开或收起；来源历史进展会反向列出引用方日期和进展，并支持展开或收起。
- 前端 32 项专项测试、代码与样式检查、正式构建及隔离原生完整自检通过；原生验收截图包含 `floating-progress-reference-picker-test.png` 和 `floating-progress-references-test.png`。
...
## 2026-09-20 悬浮窗文字即时重绘与稳定行高
- 窗口宽度改变后立即重新测量并重绘自绘文字；横向滚动消息完成后同步刷新文字、优先级链接和完成框，不再需要先选中条目。
- 简洁模式使用稳定的紧凑双行绘制高度，长条目换行不会再根据最长内容反复放大或缩小全部短条目的行高；后台数据未变化时不触发额外重绘。
- 全新隔离免安装包完整原生自检通过；专项断言覆盖缩放即时重绘、横向滚动即时重绘、换行时行高稳定及无变化后台刷新不闪动，并检查 `floating-wrapped-title-test.png` 的实际绘制结果。
