...
try {
    ...
    $packageFiles = @('Configure-TaskTrace.cmd','Configure-TaskTrace.ps1','tasktrace-settings.example.json','TaskTrace-server.exe','TaskTrace-floating.exe','Start-TaskTrace.cmd','Launch-TaskTrace.ps1','README.md','LICENSE','UPSTREAM-COMMIT.txt','SOURCE-COMMIT.txt') | ForEach-Object { Join-Path $packageRoot $_ }
