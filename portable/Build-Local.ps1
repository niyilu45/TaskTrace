...
try {
    ...
    & $compiler /nologo /target:winexe /platform:x64 /optimize+ ('/win32icon:' + (Join-Path $repoRoot 'frontend/public/favicon.ico')) ('/win32manifest:' + (Join-Path $repoRoot 'portable/FloatingWindow.manifest')) /reference:System.Windows.Forms.dll /reference:System.Drawing.dll /reference:System.IO.Compression.dll /reference:System.IO.Compression.FileSystem.dll ('/out:' + (Join-Path $packageRoot 'TaskTrace-updater.exe')) (Join-Path $repoRoot 'portable/TaskTraceUpdater.cs') (Join-Path $repoRoot 'portable/TaskTraceUpdateProgress.cs')
